...
AiOffice `0.2.0.dev1` imports DOCX packages through a bounded security scanner. It
...
Workspace imports copy source packages into `.aioffice/`; revisions and metadata are
atomically replaced. Workspace exports refuse to overwrite existing files unless the
caller explicitly passes `--overwrite`. Ambiguous external identity matches cannot be
committed.
